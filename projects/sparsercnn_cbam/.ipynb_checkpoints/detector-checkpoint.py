...
class SparseRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        nn.init.constant_(self.init_proposal_boxes.weight[:, :2], 0.5)  # 全部用0.5填充
        nn.init.constant_(self.init_proposal_boxes.weight[:, 2:], 1.0)  # 全部用1.0填充
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        for f in self.in_features:
         ...
         # CBAM fpn Feature
        features[0] = self.ca0(features[0]) * features[0]
        features[0] = self.sa0(features[0]) * features[0]

        features[1] = self.ca1(features[1]) * features[1]
        features[1] = self.sa1(features[1]) * features[1]
        
        features[2] = self.ca2(features[2]) * features[2]
        features[2] = self.sa2(features[2]) * features[2]
        
        features[3] = self.ca3(features[3]) * features[3]
        features[3] = self.sa3(features[3]) * features[3]
        ...
        output = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1]}  # 取学习到的最后一次的proposal进行最后的回归
        ...
        if self.training:
            ...
            loss_dict = self.criterion(output, targets)  # 计算各种loss并保存到字典中
            weight_dict = self.criterion.weight_dict  # 获取每种损失对应的权重参数
            for k in loss_dict.keys():
                if k in weight_dict:
                    loss_dict[k] *= weight_dict[k]  # 对应损失与对应权重系数相乘得到最终损失
            return loss_dict
