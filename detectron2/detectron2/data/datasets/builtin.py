...
_PREDEFINED_SPLITS_CITYPERSONS = dict()
_PREDEFINED_SPLITS_CITYPERSONS["citypersons"] = {
    "cityperson_train": ("cityperson/cityperson_train",
                         "cityperson/annotations/train.json"),
    "cityperson_val": ("cityperson/cityperson_val",
                       "cityperson/annotations/val.json"),
}
...
def register_all_citypersons(root):
    for dataset_name, splits_per_dataset in _PREDEFINED_SPLITS_CITYPERSONS.items():
        for key, (image_root, json_file) in splits_per_dataset.items():
            # Assume pre-defined datasets live in `./datasets`.
            register_citypersons_instances(
                key,
                _get_builtin_metadata(dataset_name),
                os.path.join(root, json_file) if "://" not in json_file else json_file,
                os.path.join(root, image_root),
...
if __name__.endswith(".builtin"):
    ...
    register_all_citypersons(_root)
