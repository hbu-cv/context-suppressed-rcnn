...
def transform_instance_annotations(
    annotation, transforms, image_size, *, keypoint_hflip_indices=None, vbox_available
):
    """
    ...
    if vbox_available:
        vbbox = BoxMode.convert(annotation["bbox_vis"], annotation["bbox_mode"], BoxMode.XYXY_ABS)
        vbbox = transforms.apply_box(np.array([vbbox]))[0].clip(min=0)
        annotation["vbbox"] = np.minimum(vbbox, list(image_size + image_size)[::-1])
...
def annotations_to_instances(annos, image_size, vbox_available, mask_format="polygon"):
    """
    ...
    if vbox_available:
        vboxes = (
            np.stack(
                [BoxMode.convert(obj["vbbox"], obj["bbox_mode"], BoxMode.XYXY_ABS) for obj in annos]
            )
            if len(annos)
            else np.zeros((0, 4))
        )
